...
router.use('/preduction', async (req, res) => {
    // ログインの確認
    let result = await isLogin(req, res, true);
    ...
    try {
        let DBResponse = await query(
            "SELECT `userPoints`.`point_id`, `userPointLabel`.`label_name` as label, `userPoints`.`grant_time` as date, `userPoints`.`point` FROM `userPoints` INNER JOIN `userPointLabel` ON `userPoints`.`label_id` = `userPointLabel`.`label_id` WHERE `userPoints`.`student_id` = ? AND `userPoints`.`deleted` = 0 AND ( `userPoints`.`point_type` = 2 ) ORDER BY `userPoints`.`grant_time` DESC, `userPoints`.`point_id` DESC LIMIT 150",
            [result],
            connection
        );
